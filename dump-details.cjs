...
async function dumpAll() {
  ...
  snapshot.forEach(doc => {
    ...
    console.log(`Opening Thumbnail: ${data.openingThumbnailUrl ? data.openingThumbnailUrl.substring(0, 50) + '...' : 'none'}`);
    console.log(`Opening Video: ${data.openingVideoUrl ? data.openingVideoUrl.substring(0, 50) + '...' : 'none'}`);
    console.log(`Hero Video: ${data.heroVideoUrl ? data.heroVideoUrl.substring(0, 50) + '...' : 'none'}`);
  });
  process.exit(0);
}
