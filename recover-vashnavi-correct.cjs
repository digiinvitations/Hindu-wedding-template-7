...
async function recover() {
  const snap = await getDoc(doc(db, "weddingConfig", "new remix template 661"));
  if (snap.exists()) {
    ...
    for (const host of hostnames) {
      ...
      console.log(`Recovered CORRECT Vijay/Vashnavi data into: ${host}`);
    }
  } else {
    console.log("Could not find 'new remix template 661'");
  }
  process.exit(0);
}
